...
class OverlayService : Service() {
    ...
    private fun ensureBleSink(): BleSink? {
        ...
        return try {
            BleSink(this, bleIdentity(), linkNote(), prefs.rumble, prefs.plainButtons, prefs.wantsConsumerControls()).also { s ->
                btSink = s
            ...
            try {
                ...
                if (prefs.targetMode == Prefs.MODE_BT) {
                    ...
                    val bt = btSink ?: if (prefs.btTransport != Prefs.TRANSPORT_CLASSIC) {
                        ...
                        BleSink(this, bleIdentity(), note, prefs.rumble, prefs.plainButtons, prefs.wantsConsumerControls()).also { s ->
                            btSink = s
                            s.open { err -> if (err.isNotEmpty()) main.post { toast(err) } }
                        }
                    } else {
