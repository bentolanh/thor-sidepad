...
class PadButtonView(
    ...
    private var zone = -1           // media unit: the third under the finger
    ...
    override fun onDraw(c: Canvas) {
        ...
        when {
            ...
            isMediaUnit(code) -> painter.drawMedia(c, 0f, 0f, width.toFloat(), height.toFloat(), if (down) zone else -1)
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        when (e.actionMasked) {
            MotionEvent.ACTION_DOWN -> if (!down) {
                down = true
                if (isMediaUnit(code)) zone = ((e.x / width.coerceAtLeast(1)) * 3f).toInt().coerceIn(0, 2)
                invalidate()
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> if (down) {
                ...
                when {
                    ...
                    isMediaUnit(code) && e.actionMasked == MotionEvent.ACTION_UP ->
                        onRelease(mediaCodeAt((z.coerceAtLeast(0) + 0.5f) / 3f))
                    isMediaUnit(code) -> {}
                    else -> unfire()
                }
            }
